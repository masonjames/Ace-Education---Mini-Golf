...
const App: React.FC = () => {
  ...
  const handleMoveEnd = useCallback(() => {
    // Only show question if the move ended and it's not a sink
    setGameState(prev => {
      if (prev.state === 'MOVING') {
    ...
    <div className="w-full h-[calc(100%-100px)] bg-black font-sans overflow-hidden relative rounded-2xl">
      ...
      {gameState.state === 'QUESTION' && gameState.currentQuestion && (
        <QuestionModal 
          ...
          onIncorrect={handleQuestionIncorrect}
        />
      )}

      {renderOverlay()}

      <InfoDialog title="Educational Mini-Golf!" goal="Line up your shot and take a swing! After every shot, answer a 4th-grade question correctly to continue your hole. Watch out for obstacles and get hints from your caddy!" onClose={() => {}} />
    </div>
